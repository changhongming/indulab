...
class CreateQuizTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('quiz_question', function (Blueprint $table) {
            $table->increments('id')->unsigned();
            $table->integer('qd_id')->references('id')->on('quiz_db')->comment('對應到quiz_db的id');
        ...
        Schema::create('quiz_choice', function (Blueprint $table) {
            $table->increments('id')->unsigned();
            $table->integer('qq_id')->references('id')->on('quiz_question')->comment('對應到quiz_question的id');
        ...
        Schema::create('quiz_answer', function (Blueprint $table) {
            $table->increments('id')->unsigned();
            $table->integer('qq_id')->references('id')->on('quiz_question')->comment('對應到quiz_question的id');
            $table->integer('qc_id')->references('id')->on('quiz_choice')->comment('對應到quiz_choice的id，表示選項內的正確答案');
            $table->timestamp('created_at')->useCurrent();
            $table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
        });
    }
